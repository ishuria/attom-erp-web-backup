  ...
  <div>
    <div class="comprehensive-table-container" style="display: flex; flex-direction: column; align-items: center; justify-content: center">
      ...
      >
        ...
        <el-table-column v-for="(prop, i) in columnsChange" :key="i" align="center" :label="prop" min-width="260" :prop="prop">
          <template #default="{ row }">
    ...
    <!--      </el-table>-->
    <!--    </div>-->
    <vab-dialog v-model="checkPersonListVisible" :before-close="handleCheckPersonClose" title="审批人选择" width="480">
      <el-form style="margin: 0">
        <el-form-item label="审批人员列表">
          <el-select v-model="reviewPersonId" clearable collapse-tags collapse-tags-tooltip placeholder="请选择审批人员">
...
const fetchData = async () => {
  ...
  const sampleRetentionRow = exchangeList.value.find((item: any) => item.column0 === 'sampleRetention')
  if (sampleRetentionRow) {
    Object.keys(sampleRetentionRow).forEach((key) => {
      if (key !== 'column0' && sampleRetentionRow[key]) {
        sampleRetentionRow[key] = sampleRetentionRow[key].split(',').map(Number)
      }
    })
  }
}
