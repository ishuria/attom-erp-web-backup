<template>
  <vab-dialog v-model="visible" :title="title" width="75%">
    <!-- 分类名输入框，仅在新增分类时显示 -->
    <el-form v-if="showGroupName" style="margin: auto 30px">
      <el-form-item label="分类名">
        <el-input v-model="groupNameValue" clearable />
      </el-form-item>
    </el-form>
...
const groupNameValue = ref(props.groupName)
...
const handleConfirm = () => {
  const data = {
    ...
    groupName: props.showGroupName ? groupNameValue.value : undefined,
  }
  emit('confirm', data)
  visible.value = false
}
