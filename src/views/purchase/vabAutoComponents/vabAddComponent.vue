<template>
  <vab-dialog v-model="dflag" style="width: fit-content" title="添加零件" top="5%" width="60%" @close="handlerCloseDialog">
    <vab-query-form>
    ...
    >
      <el-table-column class="image-wall" label="图片" width="75">
        <template #default="{ row }">
          <el-image
            ...
            @click="showPreviewImage(row.imageUrl)"
          >
      ...
      <el-table-column label="SKU" min-width="350" prop="sku">
        <template #default="{ row }">
          <span class="history-sku-text">{{ row.sku }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="历史SKU" min-width="350" prop="historySku">
        <template #default="{ row }">
