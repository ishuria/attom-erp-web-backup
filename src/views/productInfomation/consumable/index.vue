<template>
  <div class="comprehensive-table-container auto-height-container">
    ...
    />
    <vab-dialog v-model="consumableVisible" :before-close="handlerCloseDialog" class="moldDialog" title="耗材种类" width="33%">
      ...
      <div id="table-height-container">
        <el-row :gutter="20" style="display: flex; margin-bottom: 20px">
          <el-col style="flex: 6">
            <el-input
              v-model="consumableTypeForm.consumableType"
              clearable
              placeholder="请输入新增耗材种类"
              @keyup.enter="handleAddConsumableType"
            />
          </el-col>
        ...
        <el-table ref="tableRef" border :data="consumableTypeData" :header-cell-style="{ 'text-align': 'center' }" stripe>
...
const handleRowClick = (row: any) => {
  selectedRowIndex.value = row.existingPartsListId
}
const tableRowClassName = ({ row, rowIndex }: { row: any; rowIndex: number }) => {
  if (row.existingPartsListId === selectedRowIndex.value) {
    return 'select-row'
  }
  return ''
}
