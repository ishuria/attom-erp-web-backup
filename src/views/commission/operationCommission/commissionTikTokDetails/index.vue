...
import {
  ...
  getCurrencyOperationTikTokDetail,
  getCurrencyOperationTikTokSummary,
  getOperationBonusAsinSummaryList,
  getOperationBonusTikTokCommissionDetailList,
  getOperationBonusTikTokDetailList,
  getTikTokDetailMonthList,
  getTikTokOperationSiteList,
  updateCurrencyOperationTikTokDetail,
  updateCurrencyOperationTikTokSummary,
...
const resolveMonthList = (response: MonthListResponse): string[] => {
  const { data } = response
  const list = Array.isArray(data) ? data : Array.isArray(data?.list) ? data.list : Array.isArray(response.list) ? response.list : []
  return list
    .map((item) => {
      if (typeof item === 'string' || typeof item === 'number') return String(item)
      if (item && typeof item === 'object') {
        const monthItem = item as { label?: unknown; value?: unknown; month?: unknown }
        return String(monthItem.label ?? monthItem.value ?? monthItem.month ?? '')
      }
      return ''
    })
    .filter(Boolean)
...
const siteList = ref<{ id: number; label: string }[]>([{ id: -1, label: '全部' }])
const userLevelList = ref<{ id: number; label: string }[]>([{ id: -1, label: '全部' }])
const summaryMonthOption = ref<string[]>([])
const detailMonthOption = ref<string[]>([])
const bonusMonthOption = ref<string[]>([])
const currencyList = ref<{ id: number; label: string }[]>([{ id: -1, label: '原币种' }])

const resolveSort = (queryForm: SortQueryForm, data: { column: any; prop: string; order: any }) => {
...
const querySummaryData = async () => {
  ...
  try {
    summaryQueryForm.platformCode = TikTok
    ...
    console.error('查询TikTok汇总失败:', error)
...
const changeSummaryCurrency = async () => {
  await updateCurrencyOperationTikTokSummary({ currency: summaryQueryForm.currency })
  querySummaryData()
}

const fetchSummaryCurrency = async () => {
  const { data } = await getCurrencyOperationTikTokSummary()
  summaryQueryForm.currency = data ?? -1
}

const handleSummaryDetail = (row: any) => {
  // TODO: 对接 TikTok 考核指标详情接口
  void row
  indicatorList.value = []
...
const queryDetailData = async () => {
  listLoading.value = true
  try {
    const { data } = await getOperationBonusTikTokDetailList(detailQueryForm)
    detailList.value = data.list || []
    detailTotal.value = data.total || 0
  } catch (error) {
    console.error('查询TikTok明细失败:', error)
    $baseMessage('查询失败，请重试', 'error')
  } finally {
    listLoading.value = false
  }
...
const changeDetailCurrency = async () => {
  await updateCurrencyOperationTikTokDetail({ currency: detailQueryForm.currency })
  queryDetailData()
}

const fetchDetailCurrency = async () => {
  const { data } = await getCurrencyOperationTikTokDetail()
  detailQueryForm.currency = data ?? -1
}
...
const queryBonusDetailData = async () => {
  listLoading.value = true
  try {
    const { data } = await getOperationBonusTikTokCommissionDetailList(bonusDetailQueryForm)
    bonusDetailList.value = data.list || []
    bonusDetailTotal.value = data.total || 0
    bonus.value = data.totalPrice || 0
  } catch (error) {
    console.error('查询TikTok运营奖金明细失败:', error)
    $baseMessage('查询失败，请重试', 'error')
  } finally {
    listLoading.value = false
  }
...
const fetchUserLevelList = async () => {
  const { data } = await getOperationUserListByPlatform(TikTok)
...
const fetchSummaryMonthList = async () => {
  const { data } = await getAsinSummaryMonthList(TikTok)
  summaryMonthOption.value = data
  summaryQueryForm.month = data?.[0] || ''
}

const fetchDetailMonthList = async () => {
  const response = await getTikTokDetailMonthList()
  detailMonthOption.value = resolveMonthList(response)
  bonusMonthOption.value = detailMonthOption.value
  detailQueryForm.month = detailMonthOption.value[0] || ''
  bonusDetailQueryForm.month = detailMonthOption.value[0] || ''
}

const fetchSiteList = async () => {
  const { data } = await getTikTokOperationSiteList()
  siteList.value = data || []
