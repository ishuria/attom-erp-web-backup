      ...
      >
        <template #header>
          ...
          <template v-else-if="col.prop === 'anomalyCount'">
            <el-tooltip content="订发差值 - 总待售后数 - 未到货数 - 未装箱数 - 装箱数" placement="top">
              <span>
                异常数
                <el-icon><question-filled /></el-icon>
              </span>
...
const getAnomalyCount = (row: InventoryCountItem) =>
  getOrderDiffCount(row) -
  (Number(getTotalAfterCount(row)) || 0) -
  (Number(row.notYetArrived) || 0) -
  (Number(row.noEncasementCount) || 0) -
  (Number(row.encasementCount) || 0)
