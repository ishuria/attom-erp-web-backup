<template>
  <div class="comprehensive-table-container auto-height-container">
      ...
      <el-table-column label="运营分类" min-width="150" prop="typeId">
        <template #default="{ row }">
          <el-select v-model="row.typeId" placeholder="请选择运营分类" @change="handleChangeType(row)">
            <el-option v-for="item in typeList.get(String(row.userId)) ?? []" :key="item.id" :label="item.label" :value="item.id" />
          </el-select>
        </template>
      </el-table-column>
      <template #empty>
    ...
    <!-- 自动认领设定 -->
    <vab-dialog v-model="autoClaimSettingsVisible" title="自动认领设定" width="30%">
      <vab-query-form>
        <vab-query-form-left-panel>
